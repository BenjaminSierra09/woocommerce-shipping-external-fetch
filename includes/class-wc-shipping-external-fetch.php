...
class WC_Shipping_External_Fetch extends WC_Shipping_Method
{
    public function __construct($instance_id = 0)
    {
        ...
        $this->method_title       = __('External Fetch', 'woocommerce-external-fetch-shipping');
        $this->method_description = __('Description of your shipping method', 'woocommerce-external-fetch-shipping');
    ...
    public function init_instance_settings()
    {
        ...
        if (!is_array($this->instance_settings)) {
            ...
            $this->instance_settings = array_merge(
...
public function calculate_shipping( $package = array() ) {
	try {
		...
		if ( is_wp_error( $response ) ) {
			...
			$this->add_free_shipping( "E" . $response->get_error_code(), $package ); // "E" short for "Exception"
			return;
		}

		// Errors are not the fault of the customer, and we don't want to lose business, so we offer free shipping.
		if ( 200 !== $response_code ) {
			$this->add_free_shipping( "S$response_code", $package ); // "S" short for "Server error"
			return;
		}
